...
$source = <<<'PHP'
<?php
class W11PRuntimeCacheBox
{
    public mixed $value;
}

function w11p_runtime_cache_leaf($box)
{
    return $box->value;
}

function w11p_runtime_cache_root()
{
    $box = new W11PRuntimeCacheBox();
    $box->value = 21;
    return w11p_runtime_cache_leaf($box)
        + w11p_runtime_cache_leaf($box);
}
PHP;

$result = native_mir_test_compile_execute(
    $source,
    'w11p-inline-runtime-cache-frames.php',
    [],
    [
        'wave' => 11,
        'function' => 'w11p_runtime_cache_root',
