...
public:
  ...
  /// Read-only finalized relocations for an in-memory image mapper.
  std::span<const Relocation> relocations() const {
    assert(!is_virtual);
    assert(has_relocs);
...
public:
  ...
  size_t section_count() const { return sections.size(); }

  bool section_present(size_t index) const {
