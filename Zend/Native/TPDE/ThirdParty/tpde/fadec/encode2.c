...
enc_rex_mem(FeMem op0, uint64_t op1) {
    ...
    uint32_t val = op1 | op0.flags
        | ((uint32_t) op_mem_base(op0) << 8)
...
enc_mem_common(uint8_t* buf, unsigned ripoff, FeMem op0, uint64_t op1,
    ...
    if (off) {
        if (LIKELY(!disp8scale)) {
            ...
            if (!(off & ((UINT32_C(1) << disp8scale) - 1)) && op_imm_n(off >> disp8scale, 1))
...
enum {
    ...
    FE_OPC_VEX_WPP_MASK = UINT32_C(0x83) << FE_OPC_VEX_WPP_SHIFT,
    FE_OPC_VEX_MMM_SHIFT = 10,
    FE_OPC_VEX_MMM_MASK = UINT32_C(0x1f) << FE_OPC_VEX_MMM_SHIFT,
    FE_OPC_VEX_DOWNGRADE_VEX = UINT32_C(1) << 24,
    FE_OPC_VEX_DOWNGRADE_VEX_FLIPW = UINT32_C(1) << 25,
};
