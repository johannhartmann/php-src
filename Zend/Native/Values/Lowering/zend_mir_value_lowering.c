	...
	zend_mir_w06_records *records)
{
	...
	for (index = 0; index < count; index++) {
		...
		/* W01 canonical debt index 2 is refcounted_transfer (ID 1003). */
...
bool zend_mir_w06_emit_value_snapshot(
	...
	zend_mir_value_mutator *mutator)
{
	static const zend_mir_capability_id capabilities[] = {
	...
	static const zend_mir_semantic_debt_id debts[] = {
	...
	return mutator != NULL
		...
		&& zend_mir_module_commit_value_model(
			module, capabilities,
			sizeof(capabilities) / sizeof(capabilities[0]),
			debts, sizeof(debts) / sizeof(debts[0]));
}
	...
	zend_mir_value_mutator *value_mutator)
{
	...
	if (records->w05_site_count != 0
		...
		facets |= ZEND_MIR_W06_VERIFIED_CALL_MODEL;
	}
	if (!zend_mir_module_publish_w06_verifier_receipts(
				w04.module, module_fingerprint, source_fingerprint, facets)
			|| zend_mir_w06_fault_is(
	...
	result.canonical.capability_ids.offset = 0;
	result.canonical.capability_ids.count = 8;
	result.canonical.semantic_debt_ids.offset = 0;
	result.canonical.semantic_debt_ids.count = 8;
	result.canonical.codegen_eligible = false;
	result.verifier_receipts.offset = 0;
	result.verifier_receipts.count =
		values->verifier_receipt_count(values->context);
	result.modeled = true;
