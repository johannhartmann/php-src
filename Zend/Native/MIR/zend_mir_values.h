...
typedef struct _zend_mir_separation_plan_ref {
	...
	zend_mir_semantic_debt_id container_execution_debt;
...
typedef struct _zend_mir_call_transfer_ref {
	...
	zend_mir_span resolved_debt_ids;
} zend_mir_call_transfer_ref;

typedef struct _zend_mir_value_verifier_receipt_ref {
	zend_mir_verifier_receipt_id id;
...
typedef struct _zend_mir_value_view {
	...
	bool (*verifier_receipt_at)(const void *context, uint32_t index,
		zend_mir_value_verifier_receipt_ref *out);
} zend_mir_value_view;
...
typedef struct _zend_mir_value_mutator {
	...
	bool (*add_verifier_receipt)(void *context,
		const zend_mir_value_verifier_receipt_ref *record);
...
typedef enum _zend_mir_verify_w06_code {
	...
	ZEND_MIR_VERIFY_W06_RECEIPT_MISMATCH = 807,
...
#define ZEND_MIRV_TOKEN_W06_RECEIPT_MISMATCH "[MIRV0807]"
