...
class W05DefinitionTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.wave = cls.waves["W05"]
        cls.manifest = json.loads(MANIFEST_PATH.read_text(encoding="utf-8"))

    def test_w05_is_pinned_to_h_and_has_one_task(self) -> None:
        self.assertEqual(H, self.wave["expected_base_commit"])
        self.assertEqual("Direct user call-site and frame modeling", self.wave["title"])
        self.assertEqual([], self.wave["parallel_tracks"])
        self.assertEqual(["W05-integration-gate"], self.wave["required_gate_ids"])
        self.assertEqual([], self.wave["optional_gate_ids"])
        self.assertEqual(1, len(self.wave["tasks"]))
        self.assertEqual("W05-integration-gate", self.wave["tasks"][0]["task_id"])
    ...
    def test_definition_and_empty_view_are_deterministic(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="w05-empty-wave-") as temporary:
            root = Path(temporary)
        ...
        section = first[first.index("## W05"):first.index("## W06")]
        self.assertIn("W05-integration-gate", section)
