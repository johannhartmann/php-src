#!/usr/bin/env python3
"""Compile PHP source through the test-only W04 bridge and emit stable JSON."""
...
class DumpError(ValueError):
    """The W04 bridge invocation or structured response is invalid."""
...
def require_exact_keys(
    ...
    if missing:
        raise DumpError(
            "{} is missing keys: {}".format(where, ", ".join(sorted(missing)))
        )
    if unknown:
        raise DumpError(
            "{} has unknown keys: {}".format(where, ", ".join(sorted(unknown)))
        )
...
def validate_source(value: Any, filename: str, source: bytes) -> None:
    ...
    if value["filename"] != filename:
        raise DumpError("result.source.filename does not match the request")
    if value["byte_length"] != len(source):
        raise DumpError("result.source.byte_length does not match the source")
    if value["source_id"] != source_identity(filename, source):
        raise DumpError("result.source.source_id does not match the source")


def validate_diagnostics(value: Any, diagnostic_limit: int) -> None:
    if not isinstance(value, list):
        raise DumpError("result.diagnostics must be an array")
    if len(value) > diagnostic_limit:
        raise DumpError("result.diagnostics exceeds diagnostic_limit")
    ...
    for index, diagnostic in enumerate(value):
        ...
        opline = diagnostic["opline"]
        if opline is not None and (type(opline) is not int or opline < 0):
            raise DumpError("{} has an invalid opline".format(where))
        sort_key = (
            diagnostic["stage"],
            diagnostic["code"],
            opline if opline is not None else -1,
            diagnostic["message"],
        )
...
def validate_call_result(
    ...
    if value["wave"] != 4:
        raise DumpError("result.wave must be 4")
...
def main() -> int:
    ...
    parser.add_argument("--candidate", required=True, help="absolute W04 PHP path")
    ...
    try:
        ...
        stable_json_write(
            {"error": str(error), "schema_version": 1, "status": "error", "wave": 4},
            args.json_out,
        )
        print("dump-w04.py: {}".format(error), file=sys.stderr)
        return EXIT_ERROR
